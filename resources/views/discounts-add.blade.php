<x-app-layout>
    <x-slot name="header">
        <a class="opacity-50 text-slate-700" href="{{route('discounts')}}">Discounts</a> / {{ __('Add Discount') }}
    </x-slot>
    <x-slot name="title">
        {{ __('Add Discount') }}
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
            <div class="p-6 flex pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <a href="{{ route('discounts') }}" class="text-sm text-slate-700">
              ...
              <div class="p-6 overflow-x-auto">
                <form action="{{route('discounts-create')}}" method="post">
                    ...
                    <div class="mb-4 flex flex-wrap">
                        <input type="number" step="0.01" name="discount" required class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" placeholder="Discount amount" aria-label="Name">
                    </div>
                    ...
                    <div class="mb-4 flex flex-wrap">
                        <select name="patronage_id" class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" >
                            ...
                            @foreach($patronages as $patronage)
                                <option value="{{$patronage->id}}">{{$patronage->title}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4 flex flex-wrap items-center">
                        <label class="mb-2 ml-1 font-bold text-xs text-slate-700 "><input type="checkbox" name="is_overall" value=1> Apply to the whole catalogue</label>
                    </div>
