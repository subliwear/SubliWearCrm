<nav x-data="{ open: false }" class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start">
        ...
        <nav>
            ...
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              <li class="leading-normal text-sm">
                <a class="opacity-50 text-slate-700" href="/dashboard">Home</a>
              </li>
        ...
        <div class="flex items-center mt-2 grow sm:mt-0 sm:mr-6 md:mr-0 lg:flex lg:basis-auto">
            ...
            <ul class="flex flex-row justify-end pl-0 mb-0 list-none md-max:w-full ml-auto">
              ...
              <li class="flex items-center">
                ...
                <x-responsive-nav-link :href="route('profile.edit')" class="block px-0 py-2 font-semibold transition-all ease-nav-brand text-sm text-slate-500 w-auto">
                    <i class="fa fa-user sm:mr-1"></i>
                    {{ Auth::user()->name }}
                    @if(auth()->user()->is_admin())<span class="text-xs opacity-50">(Admin)</span>@endif
                    @if(auth()->user()->is_manager())<span class="text-xs opacity-50">(Designer)</span>@endif
                    @if(auth()->user()->is_customer())<span class="text-xs opacity-50">(Customer)</span>@endif
                </x-responsive-nav-link>

                @if(auth()->user()->is_admin())
                  <x-responsive-nav-link :href="route('options.index')" class="block px-0 py-2 font-semibold transition-all ease-nav-brand text-sm text-slate-500 w-auto">
                    <i class="fa fa-gear sm:mr-1"></i>
                  </x-responsive-nav-link>
                @endif
              </li>
              ...
              <li class="flex items-center px-4">
                <form method="POST" action="{{ route('logout') }}">
                  ...
                  <x-responsive-nav-link :href="route('logout')"
                      ...
                      <i fixed-plugin-button-nav class="cursor-pointer fa fa-sign-out-alt"></i>
                  </x-responsive-nav-link>
              </form>
                {{-- <a href="javascript:;" class="p-0 transition-all text-sm ease-nav-brand text-slate-500"> --}}
                  {{-- <i class="cursor-pointer fa fa-door-open"></i> --}}
