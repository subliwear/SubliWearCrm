<x-app-layout>
    <x-slot name="header">
        <a class="opacity-50 text-slate-700" href="{{route('order-statuses')}}">Order Statuses</a> / {{ __('Edit Order Status') }}
    </x-slot>
    <x-slot name="title">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
            <div class="p-6 flex pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <a href="{{ route('order-statuses') }}" class="text-sm text-slate-700">
                        <svg class="w-6 h-6 mr-2 inline-block align-middle" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                        </svg>
                        
                    </a>
              <h6>Edit Order Status</h6>
            </div>
            <div class="flex-auto px-0 pt-0 pb-2">
              <div class="p-6 overflow-x-auto">
                <form action="{{route('order-statuses-save', $order_status)}}" method="post">
                    @csrf()
                    <label class="mb-2 ml-1 font-bold text-xs text-slate-700">Title</label>
                    <div class="mb-4">
                        <input type="text" name="title" value="{{$order_status->title}}" required class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" placeholder="Status title" aria-label="Name">
                    </div>
                    ...
                    <div class="mb-4 flex flex-wrap">
                        ...
                        'text-slate-800'] as $k=>$txtclr)
                            <label class="p-4 block cursor-pointer bg-gray-200 m-4 text-center rounded-2xl">
                                <input type="radio" name="text_color" value="{{$txtclr}}" @if($order_status->text_color==$txtclr) checked @endif>
                    ...
                    <div class="mb-4 flex flex-wrap">
                        ...
                        'from-gray-400 to-gray-100'] as $k=>$bgclr)
                            <label class="p-4 block cursor-pointer bg-gray-200 m-4 text-center rounded-2xl bg-gradient-to-tl {{$bgclr}}">
                                <input type="radio" name="background_color" value="{{$bgclr}}" @if($order_status->background_color == $bgclr) checked @endif>
                            </label>
                        @endforeach 
                    </div>
